...
export default {
  ...
  methods: {
    ...
    varying vec4 v_color;
    void main() {
      if( ${_this.txxMin} < tx.x && tx.x < ${_this.txxMax} && ${_this.txyMin}< tx.y && tx.y < ${_this.txyMax} ){
         vec4 sum =v_color;
        //纵向高斯模糊
    ...
    showPreview () {
      ...
      function loop () {
        ...
        let transformX = (_this.x - _this.cropOffsertX) / scale;
        let transformY = (_this.y - _this.cropOffsertY) / scale;
        let transformZ = 0;
        obj.w = w;
        obj.h = h;
        obj.url = _this.imgs;
        obj.img = {
          width: `${_this.trueWidth}px`,
          height: `${_this.trueHeight}px`,
          transform: `scale(${scale})translate3d(${transformX}px, ${transformY}px, ${transformZ}px)rotateZ(${_this
            .rotate * 90}deg)`
        };
        obj.html = `
      <div class="show-preview" style="width: ${obj.w}px; height: ${
        ...
        <div style="width: ${w}px; height: ${h}px">
          <img src=${obj.url} style="width: ${_this.trueWidth}px; height: ${
          _this.trueHeight
          }px; transform:
          scale(${scale})translate3d(${transformX}px, ${transformY}px, ${transformZ}px)rotateZ(${_this
            .rotate * 90}deg)">
        </div>
      </div>`;
        _this.$emit("realTime", obj);
        _this.$emit("real-time", obj);

        //---------------------图片重新绘制---------------------------
        // console.log('图片重新绘制--------', 'cropX', __this.cropX, 'cropY', __this.cropY, 'cropChangeX', __this.cropChangeX, 'cropChangeY', __this.cropChangeY, 'cropOffsertX', _this.cropOffsertX, 'cropOffsertY', _this.cropOffsertY)
        // console.log('----', 'cropW', _this.cropW, 'cropH', _this.cropH, 'cropOldW', _this.cropOldW, 'cropOldH', _this.cropOldH);
        // cropW截图框框 cropH高     cropOffsertX偏移
        _this.txxMin = Math.floor(_this.cropOffsertX / _this.w * 1000) / 1000
        _this.txyMin = Math.floor(_this.cropOffsertY / _this.h * 1000) / 1000

        _this.txxMax = Math.floor((_this.cropOffsertX + _this.cropW) / _this.w * 1000) / 1000
        _this.txyMax = Math.floor((_this.cropOffsertY + _this.cropH) / _this.h * 1000) / 1000

        // console.log('xxxxxxxxxxaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', _this.txxMin, _this.txxMax, _this.txyMin, _this.txyMax);
        _this.initVideoCanvas()
        window.requestAnimationFrame(loop)
      }
